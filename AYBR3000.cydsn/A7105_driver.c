...
void A7105_init(void)
{
    ...
    A7105_WriteReg(PLL1_REG, 100 );                            // set radio channel,设置信道频率
    A7105_WriteReg(CODE1_REG, 0x0F);                       //使能CRC校验
    /*这里等待帧格式确定在设置payload length*/
    A7105_WriteReg(FIFO1_REG, 16-1); // set FIFO payload length
}
...
Bool receive_data_by_A7105(uint8 buffer_length, uint8 receive_buffer[])
{
    ...
    {
        StrobeCmd(CMD_RX);
        if (!A7105_wtr_status)          // 浮空输入脚,A7105接收开始拉高接收结束拉低
        {
            while(!RF_WTR())                // 浮空输入脚,A7105接收开始拉高接收结束拉低
            {
                A7105_CLEAR_WDT();
            }                                           //等待本次接收结束
            A7105_wtr_status = RF_WTR();
